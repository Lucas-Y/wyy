<template>
  <div class="play">
    ...
    <div
      class="song-bg"
      :style="
        `background-image: url(${songInfo &&
          songInfo.al &&
          songInfo.al
            .picUrl}?imageView&thumbnail=360y360&quality=75&tostatic=0);`
      "
    ></div>
    <!-- 播放页头部导航 -->
    <div class="header">
...
export default {
  ...
  methods: {
    async getSong() {
      const res = await getSongById(this.id)
      this.songInfo = res.songs[0]
      // 处理歌词
      const lyrContent = await getLyricById(this.id)
